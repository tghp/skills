...
For custom Gutenberg blocks. Create a class in `inc/Metaboxio/Block/` extending `AbstractBlock`. This gives you:

- `getCode()` — auto-derived from class name (e.g. `HeroSlider` → `hero-slider`)
- `getId()` — WordPress block ID (`tghp-hero-slider-block`)
- `render()` — loads template from `templates/blocks/{code}/template.php` in the plugin
- `enqueueAdminBlockAssets()` — auto-enqueues `assets/dist/block--{code}.css` for admin preview
...
The block template at `templates/blocks/hero-slider/template.php` receives `$block` (the class instance) and calls its getter methods:
...
Add the block to `Block::_getDefiners()`. The block is automatically registered in the `<name>-blocks` Gutenberg category and only explicitly defined blocks are allowed in the editor.
