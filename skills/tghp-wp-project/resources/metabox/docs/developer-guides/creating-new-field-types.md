...
## 1\. Create a new field class[​](#1-create-a-new-field-class "Direct link to 1. Create a new field class")
...
Why use the `init` hook?

We hook into `init` to make sure all Meta Box files are fully loaded and the `RWMB_Field` class is available.

## 2\. Output the field HTML[​](#2-output-the-field-html "Direct link to 2. Output the field HTML")
...
## 3\. Add the field to the builder (optional)[​](#3-add-the-field-to-the-builder-optional "Direct link to 3. Add the field to the builder (optional)")

If you want to use your custom field in the [builder](https://docs.metabox.io/creating-new-field-types/extensions/meta-box-builder/), you need to register it via the `mbb_field_types` filter:
...
4. Settings tab: `general` (default), `appearance`, or `advanced`. Optional.
...
## 4\. Use the new field[​](#4-use-the-new-field "Direct link to 4. Use the new field")
...
If your field should support [cloning](https://docs.metabox.io/creating-new-field-types/cloning-fields/), make sure your inputs have a CSS class starting with `rwmb`. The cloning script automatically detects these inputs and updates their `id` and `name` attributes correctly.

## `RWMB_Field` Class Methods[​](#rwmb%5Ffield-class-methods "Direct link to rwmb_field-class-methods")
...
### `begin_html`[​](#begin%5Fhtml "Direct link to begin_html")
...
### `end_html`[​](#end%5Fhtml "Direct link to end_html")
...
This method set field meta value before saving in database. By default it just returns the value from `$_POST`. Field class can overwrite this method to set meta value for more complicated logic ([like taxonomy\_advanced](https://github.com/wpmetabox/meta-box/blob/master/inc/fields/taxonomy-advanced.php#L22)). This method has 4 parameters:
...
### `normalize_field`[​](#normalize%5Ffield "Direct link to normalize_field")
