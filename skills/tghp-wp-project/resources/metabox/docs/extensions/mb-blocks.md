...
## Block registration with MB Builder[​](#block-registration-with-mb-builder "Direct link to Block registration with MB Builder")

The easiest way to create a block is using [MB Builder](https://docs.metabox.io/extensions/mb-blocks/extensions/meta-box-builder/). The plugin provides you with the UI to create blocks easily. This is the video on doing that:

Watch

## Block registration with block.json[​](#block-registration-with-blockjson "Direct link to Block registration with block.json")
...
### Creating the block.json file[​](#creating-the-blockjson-file "Direct link to Creating the block.json file")
...
### Registering the block[​](#registering-the-block "Direct link to Registering the block")
...
### Registering block fields[​](#registering-block-fields "Direct link to Registering block fields")
...
### Rendering the block[​](#rendering-the-block "Direct link to Rendering the block")
...
To do so, first, make sure you have the [MB Views](https://docs.metabox.io/extensions/mb-blocks/extensions/mb-views/) plugin installed and activated. Then, instead of using `file:` prefix, use `view:` instead.
...
### Block styles and scripts[​](#block-styles-and-scripts "Direct link to Block styles and scripts")
...
## Block registration without block.json[​](#block-registration-without-blockjson "Direct link to Block registration without block.json")
...
The block settings are inherited from the Block _JavaScript_ API with a few differences. See the [block registration](https://developer.wordpress.org/block-editor/developers/block-api/block-registration/) page on the Gutenberg Handbook if you need more details.

### Block settings[​](#block-settings "Direct link to Block settings")
...
#### `render_callback`[​](#render%5Fcallback "Direct link to render_callback")
...
#### `render_template`[​](#render%5Ftemplate "Direct link to render_template")
...
#### `enqueue_style`[​](#enqueue%5Fstyle "Direct link to enqueue_style")
...
If you have multiple blocks, then using multiple CSS files might hurt the performance of your website since you have many CSS files enqueued. In that case, please put the styles in your theme, and use the functions [enqueue\_block\_assets()](https://developer.wordpress.org/reference/hooks/enqueue%5Fblock%5Fassets/) and [enqueue\_block\_editor\_assets()](https://developer.wordpress.org/reference/hooks/enqueue%5Fblock%5Feditor%5Fassets/) to enqueue your styles.

#### `enqueue_script`[​](#enqueue%5Fscript "Direct link to enqueue_script")
...
#### `enqueue_assets`[​](#enqueue%5Fassets "Direct link to enqueue_assets")
...
See [MB Custom Table documentation](https://docs.metabox.io/extensions/mb-blocks/extensions/mb-custom-table/) for more details.

### Block fields[​](#block-fields "Direct link to Block fields")
...
Each field is an array of its settings. See [this guide](https://docs.metabox.io/extensions/mb-blocks/field-settings/) for details about field settings.

## Block rendering[​](#block-rendering "Direct link to Block rendering")
...
### Automatically prepare attributes[​](#automatically-prepare-attributes "Direct link to Automatically prepare attributes")

By default, all attributes inside `$attributes` are automatically prepared for you. `$attributes[ $field_id ]` returns the value of the [rwmb\_get\_value()](https://docs.metabox.io/extensions/mb-blocks/functions/rwmb-get-value/) function, which gives you all the data for the field.
...
### Accessing raw data[​](#accessing-raw-data "Direct link to Accessing raw data")
...
Note that, unlike `$attributes[ $field_id ]`, the field value in `$attributes['data']` is _not_ prepared, which means it's the raw value.

For example, for a `single_image` field, the value of `$attributes['data']['image']` is the attachment ID.

### Rendering helper functions[​](#rendering-helper-functions "Direct link to Rendering helper functions")
...
These functions work exactly like the [rwmb\_get\_value()](https://docs.metabox.io/extensions/mb-blocks/functions/rwmb-get-value/) and [rwmb\_the\_value()](https://docs.metabox.io/extensions/mb-blocks/functions/rwmb-the-value/), but applied for the current block only. The first function returns the data stored for a block field, while the 2nd one outputs that data.
...
### Reserved attributes[​](#reserved-attributes "Direct link to Reserved attributes")
...
In that case, if you want to get the block name, you can use the variable `$block->name`. `$block` is a variable that is always available in your template (or callback). It's an instance of the [WP\_Block](https://developer.wordpress.org/reference/classes/wp%5Fblock/) class.

### Deprecated parameters[​](#deprecated-parameters "Direct link to Deprecated parameters")
...
## Inner blocks[​](#inner-blocks "Direct link to Inner blocks")
...
### `allowedBlocks`[​](#allowedblocks "Direct link to allowedblocks")
...
### `templateLock`[​](#templatelock "Direct link to templatelock")
...
### Wrapper div issue[​](#wrapper-div-issue "Direct link to Wrapper div issue")
...
## Block templates[​](#block-templates "Direct link to Block templates")

Sometimes you want to load default blocks when creating a new post. Block templates allow specifying a default initial state for an editor session. Use the argument `template` when [registering the post type](https://docs.metabox.io/extensions/mb-blocks/custom-post-types/):
...
## Block data[​](#block-data "Direct link to Block data")

Each block created using MB Blocks is a _dynamic block_. By default, the block data is saved as a JSON string in the block content.
...
### Saving block data in post meta or custom tables[​](#saving-block-data-in-post-meta-or-custom-tables "Direct link to Saving block data in post meta or custom tables")
...
In this case, to prevent bugs, you _should_ set `multiple` to `false` to prevent inserting the same block multiple times (the codes above already have that).
