...
Beside [creating fields with PHP](https://docs.metabox.io/local-json/creating-fields-with-code/), you can also register custom fields with JSON. This way is more convenient and easier to manage the fields since you can put them in a separate file, use version control (Git), leverage caching, and better code editor support with JSON schema.
...
To use local JSON to register custom fields, you need to create a new folder called `mb-json` field in your theme that is writable by the web server. After that, you can put the JSON files in the folder and Meta Box will automatically load the JSON files and register the fields, no more database queries.

## JSON format[​](#json-format "Direct link to JSON format")

The JSON format is the same as when you export a field group and is very similar to the [PHP version](https://docs.metabox.io/local-json/creating-fields-with-code/) when you creating a field group, except the `$schema` and `modified` attributes:
...
## Syncing changes[​](#syncing-changes "Direct link to Syncing changes")
...
## Adding custom folders[​](#adding-custom-folders "Direct link to Adding custom folders")
