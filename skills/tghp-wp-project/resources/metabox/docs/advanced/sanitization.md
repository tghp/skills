...
## Default sanitize callbacks[​](#default-sanitize-callbacks "Direct link to Default sanitize callbacks")
...
* **[Groups](https://docs.metabox.io/sanitization/extensions/meta-box-group/)** are not sanitized due to their complexity.

## Bypass the sanitization[​](#bypass-the-sanitization "Direct link to Bypass the sanitization")
...
The instruction above uses [MB Builder](https://docs.metabox.io/sanitization/extensions/meta-box-builder/), an extension providing the UI to create fields, and is already bundled in [Meta Box Lite](https://metabox.io/lite/) and [Meta Box AIO](https://docs.metabox.io/sanitization/extensions/meta-box-aio/). If you prefer to use code, please see below.

If you use code, then set `'sanitize_callback' => 'none'` in the [field settings](https://docs.metabox.io/sanitization/field-settings/):
...
## Custom sanitize callback[​](#custom-sanitize-callback "Direct link to Custom sanitize callback")
...
| $old\_value | The old value which is in the database |
| $object\_id | The current object ID                  |

## Sanitizing subfields in a group[​](#sanitizing-subfields-in-a-group "Direct link to Sanitizing subfields in a group")
...
Why doesn't my textarea field save values?
...
[\# Link to this question](#why-doesnt-my-textarea-field-save-values)
