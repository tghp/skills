...
The Google Maps field interface

The map field settings

## Settings[​](#settings "Direct link to Settings")

In addition to the [common field settings](https://docs.metabox.io/fields/map/field-settings/), this field has the following options:

| Name                | Key               | Description                                                                                                                                                                                                                                                           |
| ------------------- | ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Google Maps API key | api\_key          | Your Google Maps API key. [Get one here](https://developers.google.com/maps/documentation/javascript/get-api-key). **Required.**                                                                                                                                      |
...
| Address field       | address\_field    | The ID of the address field. For multiple fields, separate IDs with commas. **Required.**                                                                                                                                                                             |
...
| Region              | region            | A [country code top-level domain](https://en.wikipedia.org/wiki/Country%5Fcode%5Ftop-level%5Fdomain). This helps autocomplete suggest results based on a specific region. [Learn more](https://developers.google.com/maps/documentation/geocoding/intro#RegionCodes). |
| Marker draggable    | marker\_draggable | Determines whether the marker can be dragged. Boolean, default is true. When set to false, the marker cannot be moved on the map — the location can only be set by searching for an address (Google will place the marker automatically).                             |

Address field required

You must create a [text field](https://docs.metabox.io/fields/map/fields/text/) for the address input, and pass its ID to the map's **Address field** setting.
...
### Displaying a map[​](#displaying-a-map "Direct link to Displaying a map")

To show the map on the frontend, use [rwmb\_the\_value()](https://docs.metabox.io/fields/map/functions/rwmb-the-value/) with extra parameters:
...
| marker\_icon  | URL to a custom marker icon. Optional.                                                                                         |
| marker\_title | Marker tooltip (shows on hover).                                                                                               |
| info\_window  | Content shown when clicking the marker. HTML allowed. Avoid quotes inside to prevent JavaScript issues.                        |
| js\_options   | Pass custom JavaScript options. [Reference](https://developers.google.com/maps/documentation/javascript/reference#MapOptions). |
...
### Getting the raw field value[​](#getting-the-raw-field-value "Direct link to Getting the raw field value")
...
See more about [rwmb\_get\_value()](https://docs.metabox.io/fields/map/functions/rwmb-get-value/).

### Displaying a map inside a group[​](#displaying-a-map-inside-a-group "Direct link to Displaying a map inside a group")
...
| $args     | Additional display options. Same as in rwmb\_the\_value().                                              |

## Filters[​](#filters "Direct link to Filters")

### `rwmb_google_maps_url`[​](#rwmb%5Fgoogle%5Fmaps%5Furl "Direct link to rwmb_google_maps_url")

This filter lets you modify the Google Maps script URL - for example, to add extra libraries. It receives the script URL as a parameter and should return the modified URL.
